...
with open(os.path.join(dir_path,'train_pnet_landmark.txt'),'w') as f:
    ...
    base_num=250000
    print('neg number：{} pos number：{} part number:{} total:{}'.format(len(neg),len(pos),len(part),base_num))
    ...
    print('neg number：{} pos number：{} part number:{}'.format(len(neg_keep),len(pos_keep),len(part_keep)))
    for i in pos_keep:
        f.write(pos[i])
    for i in neg_keep:
        f.write(neg[i])
